...
MODEL_TYPE="spatial-mllm-qwen3" # Qwen3 Spatial MLLM with VGGT (no connector)
PRETRAINED_MODEL_NAME_OR_PATH="Qwen/Qwen3-VL-2B-Instruct"
RUN_NAME_APPENDIX="_qwen3_phw"
# JJ: 3D Pose RoPE config for Qwen3 (PHW mode)
USE_POSE_ROPE=True  # Set to True to enable 3D Pose-aware RoPE
# USE_POSE_ROPE=False  # Set to True to enable 3D Pose-aware RoPE
POSE_ENC_TYPE="PHW"  # Pose encoding type for Qwen3: 'PHW' (Pose+H+W, recommended) or 'THW' (original)
MROPE_SECTION="24 20 20"  # Custom mrope_section for Qwen3 (default: "24 20 20" for 3D PHW)
                  # Note: Qwen3 uses 3D mRoPE with default [24, 20, 20] for head_dim=128
                  # - P/T: 24 dims
                  # - H: 20 dims
                  # - W: 20 dims
...
# Model configuration
# Note: Qwen3 Spatial MLLM uses VGGT for pose estimation only (no connector needed)
vggt_checkpoints_path=checkpoints/VGGT-1B/model.safetensors
# spatial_embeds_layer_idx and connector_type are not needed for Qwen3 version
...
entry_file=src/custom_qwen3vl/qwenvl/train/train_qwen.py
...
# Training arguments
# Note: Qwen3 Spatial MLLM does not have connector (VGGT only for pose estimation)
args="
...
# JJ: Add Pose RoPE args if enabled (for spatial-mllm-qwen3)
if [ "$USE_POSE_ROPE" = "True" ] || [ "$USE_POSE_ROPE" = "true" ]; then
    ...
    if [ -n "$MROPE_SECTION" ]; then
        ...
        echo "[Training] Qwen3 Pose RoPE enabled: pose_enc_type=${POSE_ENC_TYPE}, mrope_section=${MROPE_SECTION}"
    else
        echo "[Training] Qwen3 Pose RoPE enabled: pose_enc_type=${POSE_ENC_TYPE} (using default mrope_section)"
    fi
fi
