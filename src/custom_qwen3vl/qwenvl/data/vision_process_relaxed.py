...
#JJ: use clips shorter than 16 frames; the core to be used
def smart_nframes_relaxed(
    ele: Dict[str, Any],
    total_frames: int,
    ...
    if not (FRAME_FACTOR <= nframes and nframes <= total_frames):
        ...
        print(f"[CLAMP] {ele['video']} | target {ele.get('nframes')} -> {nframes} frames")
...
def _read_video_torchvision_relaxed(
    ele: Dict[str, Any],
    ...
    nframes = smart_nframes_relaxed(ele, total_frames=total_frames, video_fps=video_fps)
...
def _read_video_decord_relaxed(
    ele: Dict[str, Any],
) -> Tuple[torch.Tensor, float]:
...
def _read_video_torchcodec_relaxed(
    ele: Dict[str, Any],
) -> Tuple[torch.Tensor, float]:
...
# }
VIDEO_READER_BACKENDS_RELAXED = {
    "decord": _read_video_decord_relaxed,
    "torchvision": _read_video_torchvision_relaxed,
    "torchcodec": _read_video_torchcodec_relaxed,
}
...
#JJ--used function for qwen3 lazy dataset
def fetch_video_relaxed(ele: Dict[str, Any], image_patch_size: int = 14, return_video_sample_fps: bool = False,
    ...
    if isinstance(ele["video"], str):
        ...
        try:
            video, video_metadata, sample_fps = VIDEO_READER_BACKENDS_RELAXED[video_reader_backend](ele)
        except Exception as e:
            logger.warning(f"video_reader_backend {video_reader_backend} error, use torchvision as default, msg: {e}")
            video, video_metadata, sample_fps = VIDEO_READER_BACKENDS_RELAXED["torchvision"](ele)
    else:
