...
class CustomSpatialMLLMLVSMForConditionalGeneration(CustomSpatialMLLMForConditionalGeneration):
    ...
    def load_lvsm_checkpoint(self, checkpoint_path=None):
        ...
        # JJ : Zero-init connector output projections AFTER post_init/_init_weights.
        #
        # WHY: post_init → _init_weights reinitializes all nn.Linear with random normal
        # weights. This corrupts two things at step 0:
        #
        # Bug 1 — lvsm_proj random weights:
        #   fuse_for_llm adds lvsm_proj(tokens) to video_embeds.
        #   Random large output → LLM hidden states NaN → visual_hidden NaN.
        #   Fix: zero the projection weights → contribution starts at 0 (identity).
        #
        # Bug 2 — gates (nvs_gate, mod_gate) garbage value:
        #   nn.Parameter(zeros(1)) gets corrupted by _init_weights catch-all.
        #   Fix: explicitly reset to 0 after post_init.
        #
        # Bug 3 — gamma_head / beta_head must be zero-init:
        #   FiLM modulation at step 0: ctx = base * (1 + 0*gamma) + 0*beta = base
        #   Non-zero gamma/beta heads would distort the pretrained LVSM prior.
        #
        # All use the residual principle: output starts at 0, gradient drives learning.

        # Phase-3 projections (zero → identity residual add at step 0)
        nn.init.zeros_(self.connector_lvsm.lvsm_proj.weight)
        nn.init.zeros_(self.connector_lvsm.lvsm_proj.bias)
        # Phase-3 LayerNorm — re-init to canonical (weight=1, bias=0)
        # WHY: post_init() is called twice (parent + child __init__), which can
        #      corrupt LayerNorm weights to NaN.  Explicit reset ensures healthy start.
        nn.init.ones_(self.connector_lvsm.lvsm_norm.weight)
        nn.init.zeros_(self.connector_lvsm.lvsm_norm.bias)
        # Phase-5 LayerNorm — same issue
        nn.init.ones_(self.connector_lvsm.view_norm.weight)
        nn.init.zeros_(self.connector_lvsm.view_norm.bias)
        # Phase-5 view_proj — Xavier uniform (clean start, post_init may have corrupted)
        nn.init.xavier_uniform_(self.connector_lvsm.view_proj.weight)
        nn.init.zeros_(self.connector_lvsm.view_proj.bias)
        # Phase-5 FiLM heads (gamma/beta must start at 0 → identity modulation)
        nn.init.zeros_(self.connector_lvsm.gamma_head.weight)
        nn.init.zeros_(self.connector_lvsm.gamma_head.bias)
        nn.init.zeros_(self.connector_lvsm.beta_head.weight)
        nn.init.zeros_(self.connector_lvsm.beta_head.bias)
        # Gates
        with torch.no_grad():
            self.connector_lvsm.nvs_gate.zero_()
            self.connector_lvsm.mod_gate.zero_()
        print("[INFO] connector_lvsm re-initialized after post_init: "
              "lvsm_norm(1,0), lvsm_proj(0), view_norm(1,0), view_proj(xavier), "
              "gamma_head(0), beta_head(0), nvs_gate(0), mod_gate(0).")
    ...
    def _decode_nvs(self, visual_hidden, nvs_target_frames,
        ...
        """
        JJ : NVS decoding with per-view FiLM modulation.
        ...
        At init (mod_gate=0): ctx = base  (identity, pure LVSM prior).
        As training progresses: LLM shapes per-view statistics via gamma/beta.
        ...
        lvsm_context = self.connector_lvsm.modulate_lvsm_context(
            lvsm_base_context.detach().to(dtype), per_view_feat
        )  # [B, N*P, d_lvsm]
        ...
        if not torch.isfinite(lvsm_context).all():
            ...
            logger.warning(
                ...
                f"per_view_feat finite={torch.isfinite(per_view_feat).all()} "
            ...
            wandb.log(
                {
                    ...
                    "nvs/nvs_gate": self.connector_lvsm.nvs_gate.item(),
                    "nvs/mod_gate": self.connector_lvsm.mod_gate.item(),
                },
                commit=False,
            )
        except ImportError:
    ...
    def forward(
        ...
        # ============================================================
        print('*'*20, 'Phase 1: Visual Encoding', '*'*20)
        if inputs_embeds is None:
            ...
            if pixel_values_videos is not None:
                ...
                if _has_nvs_targets:
                    ...
                    input_frames = video_tchw[0][:_actual_N_in]  # [N_in, C, H, W] (strip padding)
                    target_frames_raw = nvs_target_tchw[0].to(
                        device=input_frames.device, dtype=input_frames.dtype
                    )  # [N_tgt, C, H, W]

                    # JJ : Resize target frames to match input spatial dims
                    _, _, H_in, W_in = input_frames.shape
                    if target_frames_raw.shape[2] != H_in or target_frames_raw.shape[3] != W_in:
                        target_frames_raw = F.interpolate(
                            target_frames_raw, size=(H_in, W_in),
        ...
        if labels is not None:
            ...
            _can_nvs = (
                self.enforce_LVSM
                and self.nvs_loss_weight > 0
                and _video_token_mask is not None
                and _lvsm_input_tokens is not None
            )
            if self.nvs_target_pool in ("nvs", "all"):
                _can_nvs = _can_nvs and _nvs_target_frames is not None and _target_extrinsics_w2c is not None
            ...
            if _can_nvs:
                ...
                if self.nvs_target_pool == "input":
                    ...
                    _nvs_tgt = video_tchw[0][:_actual_N_in]
                    ...
                    _input_tgt = video_tchw[0][:_actual_N_in].to(
